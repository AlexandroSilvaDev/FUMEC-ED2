...
NO* remover(NO* raiz, int chave)
{
	...
	else
	{
	    ...
	    {
	        // Nó com dois filhos
	        NO* minNo = findMin(raiz->dir);
	        raiz->chave = minNo->chave;
	        raiz->dir = remover(raiz->dir, minNo->chave);
	    }
	}
	return raiz;
}
...
NO* refreshTree(NO* raiz)
{
	...
	NO* refreshedRoot = NULL;

	// Percorre a árvore original e insere os elementos não deletados na árvore refreshedRoot
	if (!raiz->deletado)
	{
		refreshedRoot = inserir(refreshedRoot, raiz->chave);
	}

	refreshedRoot->esq = refreshTree(raiz->esq);
	refreshedRoot->dir = refreshTree(raiz->dir);
